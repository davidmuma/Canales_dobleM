...
if [ -f "Canales_dobleM.log" ]; then
	mv "Canales_dobleM.log" "Canales_dobleM.old.log" 2>>Canales_dobleM.log
fi
...
if [ $LOCAL_SCRIPT_VERSION -lt $REMOTE_SCRIPT_VERSION ]; then
	...
	wget -qO "$(basename $0)" "$URL_SCRIPT" 2>>Canales_dobleM.log
	if [ $? -eq 0 ]; then
		printf "%s$green%s$end%s\n" "[" "  OK  " "]"
		chmod +x "$(basename $0)" 2>>Canales_dobleM.log
...
case $SYSTEM in
	1)
		TVHEADEND_SERVICE="$(synoservicecfg --list | grep tvheadend)" 2>>Canales_dobleM.log #"pkgctl-tvheadend-testing"
		if [ $? -ne 0 ]; then
			SERVICES_MANAGEMENT="OLD"
		else
			SERVICES_MANAGEMENT="NEW"
		fi
		TVHEADEND_USER="$(cut -d: -f1 /etc/passwd | grep tvheadend)" 2>>Canales_dobleM.log #"tvheadend-testing"
		TVHEADEND_GROUP="$(id -gn $TVHEADEND_USER)" 2>>Canales_dobleM.log #"users"
		TVHEADEND_PERMISSIONS="700" #"u=rwX,g=,o="
		TVHEADEND_CONFIG_DIR="/var/packages/$(ls /var/packages/ | grep tvheadend)/target/var" 2>>Canales_dobleM.log #"/var/packages/tvheadend-testing/target/var"
		TVHEADEND_GRABBER_DIR="/usr/local/bin";;
	2)
		TVHEADEND_SERVICE="$(systemctl list-unit-files --type=service | grep tvheadend | tr -s ' ' | cut -d' ' -f1)" 2>>Canales_dobleM.log #"service.tvheadend42.service"
		TVHEADEND_USER="root"
		TVHEADEND_GROUP="video"
		TVHEADEND_PERMISSIONS="700" #"u=rwX,g=,o="
		TVHEADEND_CONFIG_DIR="/storage/.kodi/userdata/addon_data/$(ls /storage/.kodi/userdata/addon_data/ | grep tvheadend)" 2>>Canales_dobleM.log #"/storage/.kodi/userdata/addon_data/service.tvheadend42"
		TVHEADEND_GRABBER_DIR="/storage/.kodi/addons/$(ls /storage/.kodi/addons/ | grep tvheadend)/bin" 2>>Canales_dobleM.log;; #"/storage/.kodi/addons/service.tvheadend42/bin"
	#3)
	#	TVHEADEND_SERVICE="$(systemctl list-unit-files --type=service | grep tvheadend | tr -s ' ' | cut -d' ' -f1)" 2>>Canales_dobleM.log #"service.tvheadend42.service"
	#	TVHEADEND_USER="osmc"
	#	TVHEADEND_GROUP="video"
	...
	3) #4)
		TVHEADEND_SERVICE="$(systemctl list-unit-files --type=service | grep tvheadend | tr -s ' ' | cut -d' ' -f1)" 2>>Canales_dobleM.log #"tvheadend.service"
		TVHEADEND_USER="$(cut -d: -f1 /etc/passwd | grep -E 'tvheadend|hts')" 2>>Canales_dobleM.log #"hts"
...
if [ "$1" = "-b" -o "$1" = "-B" ]; then
	...
	case $SYSTEM in
		1)
			if [ "$SERVICES_MANAGEMENT" = "OLD" ]; then
				"/var/packages/$(ls /var/packages/ | grep tvheadend)/scripts/start-stop-status" stop 1>Canales_dobleM.log 2>&1
			else
				stop -q $TVHEADEND_SERVICE 2>>Canales_dobleM.log
			fi;;
		2)
			systemctl stop $TVHEADEND_SERVICE 2>>Canales_dobleM.log;;
		3)
			systemctl stop $TVHEADEND_SERVICE 2>>Canales_dobleM.log;;
	esac
	if [ $? -eq 0 ]; then
	...
	if [ -f "$SCRIPT_ROUTE/Backup_configuracion_Tvheadend_$(date +"%Y-%m-%d").tar.xz" ]; then
		...
		tar -cJf $SCRIPT_ROUTE/$FILE channel epggrab/xmltv input/dvb Picons 2>>Canales_dobleM.log
	else
	...
	case $SYSTEM in
		1)
			if [ "$SERVICES_MANAGEMENT" = "OLD" ]; then
				"/var/packages/$(ls /var/packages/ | grep tvheadend)/scripts/start-stop-status" start 1>Canales_dobleM.log 2>&1
			else
				start -q $TVHEADEND_SERVICE 2>>Canales_dobleM.log
			fi;;
		2)
			systemctl start $TVHEADEND_SERVICE 2>>Canales_dobleM.log;;
		3)
			systemctl start $TVHEADEND_SERVICE 2>>Canales_dobleM.log;;
	esac
	if [ $? -eq 0 ]; then
...
REMOTE_LIST_VERSION="$(curl -fLs https://github.com/davidmuma/Canales_dobleM/raw/master/version.txt | grep ^"LIST_VERSION" | cut -d'=' -f2)" 2>>Canales_dobleM.log
URL_LIST="https://github.com/davidmuma/Canales_dobleM/raw/master/files/Configuracion_Tvheadend_$REMOTE_LIST_VERSION.tar.xz" 2>>Canales_dobleM.log

REMOTE_GRABBER_VERSION="$(curl -fLs https://github.com/davidmuma/Canales_dobleM/raw/master/version.txt | grep ^"GRABBER_VERSION" | cut -d'=' -f2)" 2>>Canales_dobleM.log
URL_GRABBER="https://github.com/davidmuma/Canales_dobleM/raw/master/files/tv_grab_EPG_dobleM" 2>>Canales_dobleM.log


if [ -f $TVHEADEND_CONFIG_DIR/version.txt ]; then
	LOCAL_LIST_VERSION="$(grep ^"LIST_VERSION" $TVHEADEND_CONFIG_DIR/version.txt | cut -d'=' -f2)" 2>>Canales_dobleM.log
	LOCAL_GRABBER_VERSION="$(grep ^"GRABBER_VERSION" $TVHEADEND_CONFIG_DIR/version.txt | cut -d'=' -f2)" 2>>Canales_dobleM.log
...
if [ "$INSTALL_LIST" = true ]; then
	...
	wget -qO "Configuracion_Tvheadend_$REMOTE_LIST_VERSION.tar.xz" "$URL_LIST" 2>>Canales_dobleM.log
	if [ $? -eq 0 ]; then
		printf "%s$green%s$end%s\n" "[" "  OK  " "]"
	else
		printf "%s$red%s$end%s\n" "[" "FAILED" "]"
	...
	rm -rf $TVHEADEND_CONFIG_DIR/channel $TVHEADEND_CONFIG_DIR/input/dvb $TVHEADEND_CONFIG_DIR/Picons 2>>Canales_dobleM.log
	if [ $? -eq 0 ]; then
		printf "%s$green%s$end%s\n" "[" "  OK  " "]"
	else
		printf "%s$red%s$end%s\n" "[" "FAILED" "]"
	...
	tar -Jxf "Configuracion_Tvheadend_$REMOTE_LIST_VERSION.tar.xz" -C $TVHEADEND_CONFIG_DIR channel input Picons 2>>Canales_dobleM.log #--strip-components=1
	...
	ERROR=false
	chown -R $TVHEADEND_CHANNEL_USER:$TVHEADEND_CHANNEL_GROUP $TVHEADEND_CONFIG_DIR/channel 2>>Canales_dobleM.log
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	find $TVHEADEND_CONFIG_DIR/channel -type d -exec chmod $TVHEADEND_CHANNEL_PERMISSIONS 2>>Canales_dobleM.log {} \;
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	find $TVHEADEND_CONFIG_DIR/channel -type f -exec chmod $(($TVHEADEND_CHANNEL_PERMISSIONS-100)) 2>>Canales_dobleM.log {} \;
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	chown -R $TVHEADEND_INPUT_USER:$TVHEADEND_INPUT_GROUP $TVHEADEND_CONFIG_DIR/input 2>>Canales_dobleM.log
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	find $TVHEADEND_CONFIG_DIR/input -type d -exec chmod $TVHEADEND_INPUT_PERMISSIONS 2>>Canales_dobleM.log {} \;
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	find $TVHEADEND_CONFIG_DIR/input -type f -exec chmod $(($TVHEADEND_INPUT_PERMISSIONS-100)) 2>>Canales_dobleM.log {} \;
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	chown -R $TVHEADEND_PICONS_USER:$TVHEADEND_PICONS_GROUP $TVHEADEND_CONFIG_DIR/Picons 2>>Canales_dobleM.log
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	find $TVHEADEND_CONFIG_DIR/Picons -type d -exec chmod $TVHEADEND_PICONS_PERMISSIONS 2>>Canales_dobleM.log {} \;
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	find $TVHEADEND_CONFIG_DIR/Picons -type f -exec chmod $(($TVHEADEND_PICONS_PERMISSIONS-100)) 2>>Canales_dobleM.log {} \;
	...
	ERROR=false
	sed -i '/"chiconscheme": .*,/d' $TVHEADEND_CONFIG_DIR/config 2>>Canales_dobleM.log
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	sed -i '/"piconpath": .*,/d' $TVHEADEND_CONFIG_DIR/config 2>>Canales_dobleM.log
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	sed -i '/"piconscheme": .*,/d' $TVHEADEND_CONFIG_DIR/config 2>>Canales_dobleM.log
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	...
	sed -i "s,TVHEADEND_CONFIG_DIR,$TVHEADEND_CONFIG_DIR,g" $TVHEADEND_CONFIG_DIR/config 2>>Canales_dobleM.log
	if [ $? -eq 0 -a $ERROR = "false" ]; then
		printf "%s$green%s$end%s\n" "[" "  OK  " "]"
	else
		printf "%s$red%s$end%s\n" "[" "FAILED" "]"
...
if [ "$INSTALL_GRABBER" = true ]; then
	...
	wget -qO "tv_grab_EPG_dobleM" "$URL_GRABBER" 2>>Canales_dobleM.log
	if [ $? -eq 0 ]; then
		printf "%s$green%s$end%s\n" "[" "  OK  " "]"
	else
		printf "%s$red%s$end%s\n" "[" "FAILED" "]"
	...
	if [ -f /usr/bin/tv_grab_EPG_dobleM -a $SYSTEM -eq 1 ]; then
		rm /usr/bin/tv_grab_EPG_dobleM 2>>Canales_dobleM.log
	fi
	ERROR=false
	rm -rf $TVHEADEND_CONFIG_DIR/epggrab/xmltv 2>>Canales_dobleM.log
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	tar -Jxf "Configuracion_Tvheadend_$REMOTE_LIST_VERSION.tar.xz" -C $TVHEADEND_CONFIG_DIR epggrab 2>>Canales_dobleM.log
	if [ $? -ne 0 -a $SYSTEM -ne 2 ]; then
		ERROR=true
	fi
	if [ $SYSTEM -ne 1 ]; then
		sed -i -- "s,/usr/local/bin,$TVHEADEND_GRABBER_DIR,g" $TVHEADEND_CONFIG_DIR/epggrab/xmltv/channels/* 2>>Canales_dobleM.log
		if [ $? -ne 0 ]; then
			ERROR=true
		fi
	fi
	chown -R $TVHEADEND_EPGGRAB_USER:$TVHEADEND_EPGGRAB_GROUP $TVHEADEND_CONFIG_DIR/epggrab 2>>Canales_dobleM.log
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	find $TVHEADEND_CONFIG_DIR/epggrab -type d -exec chmod $TVHEADEND_EPGGRAB_PERMISSIONS 2>>Canales_dobleM.log {} \;
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	find $TVHEADEND_CONFIG_DIR/epggrab -type f -exec chmod $(($TVHEADEND_EPGGRAB_PERMISSIONS-100)) 2>>Canales_dobleM.log {} \;
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	if [ ! -d $TVHEADEND_GRABBER_DIR ]; then
		mkdir -p $TVHEADEND_GRABBER_DIR 2>>Canales_dobleM.log
	fi
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	cp tv_grab_EPG_dobleM $TVHEADEND_GRABBER_DIR/ 2>>Canales_dobleM.log
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	chown $TVHEADEND_USER:$TVHEADEND_GROUP $TVHEADEND_GRABBER_DIR/tv_grab_EPG_dobleM 2>>Canales_dobleM.log
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	chmod $(($TVHEADEND_PERMISSIONS-100)) $TVHEADEND_GRABBER_DIR/tv_grab_EPG_dobleM 2>>Canales_dobleM.log
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	chmod +rx $TVHEADEND_GRABBER_DIR/tv_grab_EPG_dobleM 2>>Canales_dobleM.log
	if [ $? -eq 0 -a $ERROR = "false" ]; then
		printf "%s$green%s$end%s\n" "[" "  OK  " "]"
	else
		printf "%s$red%s$end%s\n" "[" "FAILED" "]"
	...
	ERROR=false
	sed -i '/tv_grab_EPG_dobleM/,/},/d' $TVHEADEND_CONFIG_DIR/epggrab/config 2>>Canales_dobleM.log
	if [ $? -ne 0 ]; then
		ERROR=true
	fi
	...
	sed -i "s,TVHEADEND_GRABBER_DIR,$TVHEADEND_GRABBER_DIR,g" $TVHEADEND_CONFIG_DIR/epggrab/config 2>>Canales_dobleM.log
	if [ $? -eq 0 -a $ERROR = "false" ]; then
		printf "%s$green%s$end%s\n" "[" "  OK  " "]"
	else
		printf "%s$red%s$end%s\n" "[" "FAILED" "]"
...
if [ -f "Configuracion_Tvheadend_$REMOTE_LIST_VERSION.tar.xz" ]; then
	rm "Configuracion_Tvheadend_$REMOTE_LIST_VERSION.tar.xz" 2>>Canales_dobleM.log
fi
if [ -f "tv_grab_EPG_dobleM" ]; then
	rm "tv_grab_EPG_dobleM" 2>>Canales_dobleM.log
fi


if [ -f "Canales_dobleM.old.log" -a ! -s Canales_dobleM.old.log ]; then
	rm "Canales_dobleM.old.log" 2>>Canales_dobleM.log
fi
if [ -f "Canales_dobleM.log" -a ! -s Canales_dobleM.log ]; then
	rm "Canales_dobleM.log" 2>>Canales_dobleM.log
fi
