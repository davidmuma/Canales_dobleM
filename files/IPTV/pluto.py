...
class PlutoHLSStreamWriter(HLSStreamWriter):
    def __init__(self, *args, **kwargs):
        ...
        adverts = r"_ad/creative/|dai\.google\.com"
        bumpers = r"|Pluto_TV_OandO/.*Bumper"
        if self.stream.disable_bumpers:
            adverts += bumpers
        self.ad_re = re.compile(adverts)
...
class PlutoHLSStream(HLSStream):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.disable_bumpers = self.session.get_plugin_option("pluto", "disable_bumpers")
...
class Pluto(Plugin):
    arguments = PluginArguments(
        PluginArgument(
            ...
            action="store_true",
            help="""
            In addition to the adverts, also filter out the bumper segments. These are
            the short video sections that appear before and after advert breaks.
            """
        ),
    )
