...
INFO_CARPETA_TVH="echo /var/packages/$(ls /var/packages/ | grep tvheadend)/target/var" #Linux /home/hts/.hts/tvheadend #Synology /var/packages/tvheadend/target/var #Libreelec /storage/.kodi/userdata/addon_data/service.tvheadend43 #Alexelec /storage/.config/tvheadend
INFO_CARPETA_GRABBER="/usr/local/bin"   #"$(which tvheadend | sed 's/\/tvheadend//')"

SYSTEM=1   #1-Synology/XPEnology   2-LibreELEC/OpenELEC   3-Linux
...
PARAR_TVHEADEND()
{
	case $SYSTEM in
		...
		3)
			systemctl stop $TVHEADEND_SERVICE 2>>i_dobleM.log;; #service tvheadend stop
	esac
	if [ $? -eq 0 ]; then
...
INICIAR_TVHEADEND()
{
	case $SYSTEM in
		...
		3)
			systemctl start $TVHEADEND_SERVICE 2>>i_dobleM.log;; #service tvheadend start
	esac
	if [ $? -eq 0 ]; then
		printf "%s$green%s$end%s\n" "[" "  OK  " "]"
	else
